...
public interface SquadRepository extends JpaRepository<Squad, Long> {
    ...
    @Query("SELECT s FROM Squad s LEFT JOIN FETCH s.squadPositions sp LEFT JOIN FETCH sp.teamMember tm LEFT JOIN FETCH tm.user u LEFT JOIN FETCH u.userInfo ui WHERE s.team.id = :teamId AND s.formationType = :formationType") Optional<Squad> findByTeamIdAndFormationTypeWithPositionsAndMembers(@Param("teamId") Long teamId, @Param("formationType") String formationType);
